...
export const sendEmailToResetPassword = async (req, res, user) => {
  try {
    ...
    const resetUserToken = encodeURIComponent(jwt.sign(
      { email: email }, 
      process.env.JWT_SECRET_KEY, 
      { expiresIn: '1h' }
      ));
    ...
    const emailOptions = {
      ...
      return res.status(201).json({message:"check your email to reset password"});
  ...
  export const resetUserPassword = async(req,res) =>{
    ...
    try {
      ...
      try {
        await passwordValidation.validateAsync({ password: newPassword, confirmPassword });
